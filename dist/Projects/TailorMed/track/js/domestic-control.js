// Domestic Timeline 控制邏輯
// 專門處理 Domestic timeline 的狀態切換

class DomesticControl {
  constructor() {
  ...
  init() {
    ...
    if (window.TimelineManager) {
      ...
      console.log('Domestic Control 初始化完成');
    } else {
  ...
  bindControlButtons() {
    const buttons = document.querySelectorAll(
      '.domestic-control-panel .control-btn'
    );
  ...
  switchTimeline(status) {
    ...
    try {
      ...
      console.error('切換 Domestic Timeline 時發生錯誤:', error);
    }
  }
  ...
  updateStatusDisplay(status) {
    ...
    const statusElement = document.querySelector(
      '.domestic-control-panel .status-value'
    );
    if (statusElement) {
  ...
  getStatusDisplayName(status) {
    const statusNames = {
      'domestic-step1': 'Domestic 第一步',
      'domestic-step2': 'Domestic 第二步',
      'domestic-step3': 'Domestic 第三步',
      'domestic-step4': 'Domestic 第四步',
      completed: 'Domestic 完成',
    };
  ...
  renderTimelineByStatus(status) {
    ...
    try {
      ...
      console.error('渲染 Domestic Timeline 時發生錯誤:', error);
    }
  }
  ...
  // 重置到初始狀態
  resetTimeline() {
    this.switchTimeline('domestic-step1');
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  // 延遲初始化，確保所有腳本都載入完成
  setTimeout(() => {
    window.domesticControl = new DomesticControl();
  }, 200);
});
